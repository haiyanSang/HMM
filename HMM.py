...
def viterbi(obs, states, start_p, trans_p, emit_p) :
    ...
    for y in states: #init
        ...
        print(y)
        print(emit_p.get(obs[0],0.00000001))
        path[y] = [y]
...
def validate():
    ...
    print("测试语料:%d行，总词数:%d,正确标注总数:%d,正确率:%d",line_sum,words_count,right,correct_rate)
